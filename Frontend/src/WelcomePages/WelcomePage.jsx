...
const GlobalStyles = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes glowPulse{0%,100%{box-shadow:0 0 24px rgba(217,119,6,0.25)}50%{box-shadow:0 0 48px rgba(217,119,6,0.55)}}
    ...
    .section-tag{display:inline-flex;align-items:center;gap:7px;background:rgba(217,119,6,.1);border:1px solid rgba(217,119,6,.3);color:#D97706;padding:5px 16px;border-radius:24px;font-size:.72rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;margin-bottom:1rem;font-family:'Lato',sans-serif}
...
const SectionHeading = ({ tag, title, subtitle }) => {
  ...
  return (
    <div ref={ref} style={{ textAlign:'center', marginBottom:'3.5rem', opacity:visible?1:0, transform:visible?'translateY(0)':'translateY(28px)', transition:'opacity .7s ease,transform .7s ease' }}>
      ...
      <div style={{ height:3, width:0, background:`linear-gradient(90deg,${AMBER},#F59E0B)`, borderRadius:2, margin:'1.2rem auto 0', animation:'drawLine 1s ease .3s forwards' }} />
    </div>
  );
};
...
const StatCard = ({ value, suffix='', label, delay }) => {
  ...
  return (
    <div ref={ref} style={{ textAlign:'center', opacity:visible?1:0, transform:visible?'translateY(0)':'translateY(24px)', transition:`opacity .7s ease ${delay}ms,transform .7s ease ${delay}ms` }}>
      <div style={{ fontSize:'2.4rem', fontWeight:800, color:AMBER, fontFamily:SERIF, lineHeight:1 }}>{visible?`${num}${suffix}`:`0${suffix}`}</div>
...
const FeatureCard = ({ icon, title, desc, delay }) => {
  ...
  return (
    <div ref={ref} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{ background:'#fff', border:`1px solid ${hov?'rgba(217,119,6,.45)':'#E2E8F0'}`, borderRadius:20, padding:'2rem', boxShadow:hov?'0 24px 56px rgba(30,41,59,.13)':'0 2px 16px rgba(30,41,59,.05)', transform:visible?(hov?'translateY(-8px)':'translateY(0)'):'translateY(32px)', opacity:visible?1:0, transition:`opacity .7s ease ${delay}ms,transform .4s ease,box-shadow .4s,border .3s`, position:'relative', overflow:'hidden', cursor:'default' }}>
      <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:hov?`linear-gradient(90deg,${AMBER},#F59E0B)`:'transparent', transition:'background .4s', borderRadius:'20px 20px 0 0' }} />
      <div style={{ width:58, height:58, borderRadius:16, background:hov?`linear-gradient(135deg,${AMBER},${AMBER_DARK})`:'rgba(217,119,6,.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.7rem', marginBottom:'1.25rem', transition:'background .4s,box-shadow .4s', boxShadow:hov?'0 8px 24px rgba(217,119,6,.35)':'none' }}>{icon}</div>
...
const CourseCard = ({ icon, name, desc, index }) => {
  ...
  const [hov, setHov] = useState(false);
  return (
    <div ref={ref} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{ background:hov?`linear-gradient(135deg,${AMBER},${AMBER_DARK})`:'#fff', border:`1px solid ${hov?AMBER:'#E2E8F0'}`, borderRadius:18, padding:'1.75rem', opacity:visible?1:0, transform:visible?(hov?'translateY(-6px) scale(1.02)':'translateY(0)'):'translateY(24px)', transition:`opacity .6s ease ${(index%4)*70}ms,transform .35s ease,background .4s,border .3s,box-shadow .35s`, cursor:'pointer', boxShadow:hov?'0 20px 48px rgba(30,41,59,.22)':'0 2px 12px rgba(30,41,59,.05)' }}>
...
const TestimonialCard = ({ name, role, text, delay }) => {
  ...
  return (
    <div ref={ref} style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:20, padding:'2rem', opacity:visible?1:0, transform:visible?'translateY(0)':'translateY(28px)', transition:`opacity .7s ease ${delay}ms,transform .7s ease ${delay}ms`, boxShadow:'0 4px 20px rgba(30,41,59,.06)', position:'relative' }}>
      <div style={{ position:'absolute', top:20, right:24, fontSize:'3rem', color:'rgba(217,119,6,.12)', fontFamily:'Georgia,serif', lineHeight:1 }}>"</div>
      <div style={{ color:AMBER, fontSize:'1rem', marginBottom:'.75rem', letterSpacing:2 }}>★★★★★</div>
      <p style={{ color:'#475569', fontSize:'.9rem', lineHeight:1.8, marginBottom:'1.25rem', fontStyle:'italic', fontFamily:SANS }}>"{text}"</p>
      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
        <div style={{ width:42, height:42, borderRadius:'50%', background:`linear-gradient(135deg,${AMBER},${AMBER_DARK})`, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:'1rem', fontFamily:SERIF }}>{name[0]}</div>
        <div>
...
const RaadhyamHomepage = () => {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F8FAFC', fontFamily:SANS }}>
          ...
          <div className="hero-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'4rem', alignItems:'center' }}>
            ...
            <div className="hl">
              ...
              <h1 style={{ fontFamily:SERIF, fontSize:'clamp(2.8rem,5vw,4.2rem)', fontWeight:700, lineHeight:1.1, color:SLATE, marginBottom:'1.2rem', letterSpacing:'-0.02em' }}>
                Discover Your{' '}
                <span style={{ background:`linear-gradient(90deg,${AMBER},#F59E0B,${AMBER})`, backgroundSize:'200% auto', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', animation:'shimmer 3s linear infinite' }}>Musical</span>
                <br />Identity
              </h1>
              <div style={{ height:3, width:0, background:`linear-gradient(90deg,${AMBER},#F59E0B)`, borderRadius:2, marginTop:14, animation:'drawLine 1s ease .7s forwards' }} />
            ...
            <div className="hr hero-img" style={{ position:'relative', display:'flex', justifyContent:'center', alignItems:'center' }}>
              <div style={{ position:'absolute', width:'85%', height:'85%', borderRadius:'60% 40% 55% 45% / 50% 60% 40% 50%', background:`linear-gradient(135deg,rgba(217,119,6,.15),rgba(251,191,36,.1))`, zIndex:0 }} />
              <div style={{ position:'relative', zIndex:2, borderRadius:28, overflow:'hidden', boxShadow:`0 32px 80px rgba(217,119,6,.2),0 8px 32px rgba(30,41,59,.12)`, border:`3px solid rgba(217,119,6,.3)`, maxWidth:420, width:'100%' }}>
                ...
                <div style={{ position:'absolute', bottom:18, left:18, background:'rgba(255,255,255,.95)', borderRadius:12, padding:'10px 16px', boxShadow:'0 4px 16px rgba(0,0,0,.12)', display:'flex', alignItems:'center', gap:10 }}>
                  <div style={{ width:36, height:36, borderRadius:'50%', background:`linear-gradient(135deg,${AMBER},${AMBER_DARK})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.1rem' }}>🎵</div>
                  <div>
                    <div style={{ fontSize:'.78rem', fontWeight:800, color:SLATE, fontFamily:SANS }}>500+ Students</div>
                    <div style={{ fontSize:'.68rem', color:AMBER, fontWeight:700, fontFamily:SANS }}>★★★★★ 5.0 Rated</div>
                  </div>
                </div>
                <div style={{ position:'absolute', top:16, right:16, background:`linear-gradient(135deg,${AMBER},${AMBER_DARK})`, color:'#fff', borderRadius:10, padding:'6px 14px', fontSize:'.75rem', fontWeight:700, letterSpacing:'.06em', boxShadow:'0 4px 14px rgba(217,119,6,.4)', fontFamily:SANS }}>LIVE CLASSES</div>
              </div>
              <div style={{ position:'absolute', top:'8%', left:'-5%', zIndex:3, background:'#fff', borderRadius:14, padding:'12px 16px', boxShadow:'0 8px 28px rgba(30,41,59,.12)', border:`1px solid rgba(217,119,6,.2)`, animation:'floatNote 5s ease-in-out .5s infinite' }}>
                <div style={{ fontSize:'1.4rem', marginBottom:4 }}>🎸</div>
                <div style={{ fontSize:'.72rem', fontWeight:700, color:SLATE, fontFamily:SANS }}>25+ Instruments</div>
              </div>
              <div style={{ position:'absolute', bottom:'10%', right:'-4%', zIndex:3, background:'#fff', borderRadius:14, padding:'12px 16px', boxShadow:'0 8px 28px rgba(30,41,59,.12)', border:`1px solid rgba(217,119,6,.2)`, animation:'floatNote 6s ease-in-out 1.5s infinite' }}>
                <div style={{ fontSize:'1.4rem', marginBottom:4 }}>🏆</div>
        ...
        <div style={{ maxWidth:1280, margin:'0 auto', position:'relative', zIndex:2 }}>
          ...
          <div className="founder-grid" style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:'5rem', alignItems:'center' }}>
            ...
            <div style={{ position:'relative', display:'flex', justifyContent:'center' }}>
              ...
              <div style={{ width:260, height:260, borderRadius:'50%', border:`3px solid rgba(217,119,6,.35)`, overflow:'hidden', position:'relative', zIndex:2, boxShadow:'0 24px 64px rgba(0,0,0,.2)', background:`linear-gradient(135deg,rgba(217,119,6,.15),rgba(30,41,59,.5))` }}>
      ...
      <section style={{ padding:'7rem 2rem', background:'#F8FAFC' }}>
        <div style={{ maxWidth:1280, margin:'0 auto' }}>
          ...
          <div style={{ textAlign:'center', marginTop:'3rem' }}>
            <Link to="/Courses" className="amber-btn" style={{ padding:'13px 40px' }}>View All Courses</Link>
          </div>
        </div>
      </section>
      ...
      {/* ══ TESTIMONIALS ═════════════════════════════════════════════════ */}
      <section style={{ padding:'7rem 2rem', background:'#fff', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', left:'50%', top:'50%', transform:'translate(-50%,-50%)', width:700, height:700, borderRadius:'50%', background:'radial-gradient(circle,rgba(217,119,6,.07) 0%,transparent 70%)', pointerEvents:'none' }} />
        <div style={DOT_PAT} />
        <div style={{ maxWidth:1280, margin:'0 auto', position:'relative', zIndex:2 }}>
      ...
      <section style={{ padding:'7rem 2rem', background:'#F8FAFC', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:760, margin:'0 auto', textAlign:'center', position:'relative', zIndex:2 }}>
          <div style={{ width:80, height:80, borderRadius:'50%', background:`linear-gradient(135deg,${AMBER},${AMBER_DARK})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'2.2rem', margin:'0 auto 2rem', boxShadow:'0 12px 36px rgba(217,119,6,.4)', animation:'glowPulse 3s ease-in-out infinite' }}>🎵</div>
          ...
          <h2 style={{ fontFamily:SERIF, fontSize:'clamp(2.2rem,4vw,3.2rem)', fontWeight:700, color:SLATE, letterSpacing:'-0.02em', marginBottom:'1rem', marginTop:'.5rem' }}>
            Ready to Begin Your{' '}
            <span style={{ background:`linear-gradient(90deg,${AMBER},#F59E0B,${AMBER})`, backgroundSize:'200% auto', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', animation:'shimmer 3s linear infinite' }}>Musical Journey?</span>
          </h2>
          <div style={{ height:3, width:72, background:`linear-gradient(90deg,${AMBER},#F59E0B)`, borderRadius:2, margin:'0 auto 1.5rem' }} />
          ...
          <div style={{ display:'flex', gap:16, justifyContent:'center', flexWrap:'wrap' }}>
            <Link to="/login" className="amber-btn" style={{ fontSize:'1.05rem', padding:'15px 44px' }}>Enroll Now</Link>
